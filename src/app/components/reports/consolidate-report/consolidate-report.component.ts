...
import { CONSOLIDATED_REPORT_LIST } from 'src/app/TableHeader';
...
export class ConsolidateReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.tableHeader = CONSOLIDATED_REPORT_LIST;
  ...
  getStockReports() {
    ...
    this.common.getConsolidatedVisitReport(req).subscribe((res: any) => {
      console.log("consolidated Report response::", res);
  ...
  onDownload() {
    ...
    this.common.getConsolidatedVisitReportDownload(req).subscribe(response => {
      let res: any = response;
      console.log("cons report Response for download>>>>>>>>>>>>>", res);
      if (res.respondcode == 200) {
        console.log("Report path:", res.data)

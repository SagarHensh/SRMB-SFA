...
import { NEW_SHOP_REGISTRATION_LIST } from 'src/app/TableHeader';

@Component({
  selector: 'app-new-shop-registration-report',
  templateUrl: './new-shop-registration-report.component.html',
  styleUrls: ['./new-shop-registration-report.component.css']
})
export class NewShopRegistrationReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.tableHeader = NEW_SHOP_REGISTRATION_LIST;
  ...
  getReports() {
    ...
    this.common.getNewShopRegReport(req).subscribe((res: any) => {
      // console.log("New Shop Reg Report response::", res);
      if (res.respondcode == 200) {
        let respObj: any = res.data;
        if (respObj.list.length > 0) {
          this.allReportList = respObj.list;
          this.totalRecords = respObj.count.length == 0 ? 0 : respObj.count[0].totalCount;
  ...
  onDownload() {
    ...
    this.common.getNewShopRegReportDownload(req).subscribe(response => {
      let res: any = response;
      // console.log("unique report Response for download>>>>>>>>>>>>>", res);
      if (res.respondcode == 200) {
        var file_path = app_config.downloadUrlExcel + res.data;
